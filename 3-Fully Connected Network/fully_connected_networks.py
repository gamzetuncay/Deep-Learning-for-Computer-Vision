...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores, l2_cache = Linear.forward(l1_out, self.params['W2'], self.params['b2'])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    reg = self.reg * (torch.sum(torch.square(self.params['W1'])) + torch.sum(torch.square(self.params['W2'])))
    loss += reg
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    - seed: If not None, then pass this random seed to the dropout layers. This
      ...
      model.
    - dtype: A torch data type object; all computations will be performed using
      this datatype. float is faster but less accurate, so you sdhould use
      double for numeric gradient checking.
    - device: device to use for computation. 'cpu' or 'cuda'
    """
    ...
    for lay, hd in enumerate(hidden_dims):
      W_index='W{}'.format(lay + 1)
      b_index='b{}'.format(lay + 1)
      self.params[W_index] = weight_scale * torch.randn(layer_input_dim, hd, device=device, dtype = self.dtype)
      self.params[b_index] = weight_scale * torch.zeros(hd, device=device, dtype=self.dtype)
      layer_input_dim = hd

    self.params['W{}'.format(self.num_layers)] = weight_scale * torch.randn(layer_input_dim, num_classes, device=device, dtype = self.dtype)
    self.params['b{}'.format(self.num_layers)] = weight_scale * torch.zeros(num_classes, device=device, dtype = self.dtype)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for lay in range(layers):
      ...
      scores, cache = Linear_ReLU.forward(scores, self.params[W_index], self.params[b_index])
    ...
    W_index='W{}'.format(lay + 1)
    b_index='b{}'.format(lay + 1)
    scores, cache = Linear.forward(scores, self.params[W_index], self.params[b_index])
    total_cache.append(cache)
    ...
    while lay >= 0:
      ...
      W_index='W{}'.format(lay + 1)
      b_index='b{}'.format(lay + 1)
      dx, grads[W_index], grads[b_index] = Linear_ReLU.backward(dx, total_cache.pop())
      grads[W_index] += 2 * self.reg * grads[W_index]
      lay -= 1
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver = Solver(model, data_dict, update_rule = sgd, optim_config = {'learning_rate': 1e-1,},
                    lr_decay = 0.95, num_epochs = 30, batch_size = 100, print_every = 100, device = device)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return solver
