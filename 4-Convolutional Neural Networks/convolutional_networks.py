...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    padded_x = torch.nn.functional.pad(x, (pad, pad, pad, pad)) # zero-padding for input
      ...
      for j in range(F):
        for h in range(H_out):
          for k in range(W_out):
              block = padded_x[i, :, h * stride:h * stride + HH, k * stride:k * stride + WW]
      ...
      for j in range(F):
        ...
        for h in range(0, H_out):
          for k in range(0, W_out):
            ...
            dx_pad[i, :, h * stride:h * stride + HH, k * stride:k * stride + WW] += w[j] * dout[i, j, h, k]
  ...
  def forward(x, pool_param):
    ...
    for i in range(H_out):
      for j in range(W_out):
        # Apply max pooling along both width and height and assign the result
  ...
  def backward(dout, cache):
    ...
    for i in range(N):
      for j in range(C):
        for h in range(H_out):
          for k in range(W_out):
            block = x[i, j, h * stride:h * stride + pool_height, k * stride:k * stride + pool_width]
