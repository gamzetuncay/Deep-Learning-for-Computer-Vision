      ...
      for rs in regularization_strengths:
        for lr_decay in learning_rate_decays:
          ...
          stats = net.train(data_dict['X_train'], data_dict['y_train'],data_dict['X_val'], data_dict['y_val'],\
                    ...
                    learning_rate=lr, learning_rate_decay=lr_decay, reg=rs, verbose=False)
          
          # Get the validation accuracy
          val_acc = stats['val_acc_history'][-1]
          
          # If the validation accuracy is better than the current best, update the best parameters
