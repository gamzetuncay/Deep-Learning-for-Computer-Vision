...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      accuracy_fold = knn_classifier.check_accuracy(x_validation_fold, y_validation_fold, k)
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k, accuracy_per_fold in k_to_accuracies.items():
    mean_accuracy = statistics.mean(accuracy_per_fold)
    if mean_accuracy > best_accuracy:
      best_accuracy, best_k = mean_accuracy, k 
